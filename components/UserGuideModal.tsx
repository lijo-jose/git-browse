...
const SECTIONS = [
  {
    ...
    items: [
      ...
      { key: 'U', desc: 'Pull from remote' },
      { key: 'P', desc: 'Push to remote' },
      { key: 'T', desc: 'Create a new tag' },
      { key: '⌘K / Ctrl+K', desc: 'Open command palette — search all actions' },
    ],
  },
  {
    title: 'Panels',
    items: [
      { key: 'Explorer', desc: 'Left sidebar — browse and pin local git repos, organised into groups' },
      ...
      { key: 'Activity bar', desc: 'Bottom bar — shows the last git command and its status; click to expand the full operation log' },
    ],
  },
  {
    ...
    items: [
      ...
      { key: 'Pull', desc: 'Pull latest commits from remote (guarded by danger zone on first use)' },
      { key: 'Push', desc: 'Push local commits to remote; picks a remote picker when you have more than one remote' },
      { key: 'Tag', desc: 'Create and push a new tag to the remote (guarded by danger zone on first use)' },
      { key: '🔒 / 🔓', desc: 'Lock/unlock all danger-zone operations at once' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'Right-click', desc: 'Merge into current, Rebase onto, Interactive rebase, or Delete' },
      { key: 'Rebase', desc: 'Rewrites history — guarded by danger zone on first use' },
      { key: 'diff button', desc: 'Instantly compare that branch against HEAD in Git Compare' },
    ],
  },
  {
    title: 'Changes Tab',
    items: [
      ...
      { key: 'Push', desc: 'Push commits to remote; shows a remote picker when multiple remotes exist' },
      { key: 'Patch ▾', desc: 'Download all or selected changes as a .patch file, or import a patch with git apply' },
    ],
  },
  {
    title: 'Danger Zone',
    items: [
      { key: 'Proceed once', desc: 'Run the guarded operation this one time; stays locked for next time' },
      { key: 'Always allow', desc: 'Run now and unlock this operation so future runs skip the dialog' },
      { key: 'Per-operation', desc: 'Push, Pull, Tag, and Rebase each have independent lock state' },
      { key: 'Persistent', desc: 'Unlock state is saved in local storage and survives reloads' },
    ],
  },
  {
    title: 'Info Tab',
    items: [
      { key: 'Remotes', desc: 'View configured remote URLs; add, edit, or remove remotes' },
      { key: 'HEAD', desc: 'Current branch and commit SHA' },
      { key: 'Settings ⚙', desc: 'Open Repository Settings — git identity, remotes, danger zone' },
    ],
  },
  {
    title: 'Tips',
    items: [
      ...
      { key: 'Groups', desc: 'Organise pinned repos into named groups in the Explorer' },
      { key: 'Clone', desc: 'Clone a remote repository directly from the Explorer sidebar' },
      { key: 'Error hints', desc: 'When a git command fails, Git Browse shows a plain-English suggestion in the activity bar' },
